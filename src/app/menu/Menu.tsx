  ...
  return (
    <section className="bg-menubg mt-8 flex w-full max-w-[1300px] flex-row">
      ...
      <div className="flex w-full flex-col gap-4 md:w-4/6">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 rounded bg-transparent px-3 py-2 font-medium text-[#fff] hover:text-primary"
        >
          <ArrowLeft className="h-5 w-5" />
          <span>Back</span>
        </button>
        <div
          className="relative hidden h-[30vh] w-full bg-[#fff] md:flex"
          style={{
            background:
            ...
            {organizedMenu.map((category) => {
              return category.items.length > 0 ? (
                ...
                >
                  <h2
                    className={cn(
                      "pb-4 text-2xl font-bold text-[#fff]",
